...
2. RESOLVED (`0015_privacy_hardening.sql`): `profiles_select_public` (and `profiles_select_teammates`, added by 0007, same problem) exposed the ENTIRE row of opted-in/teammate users — email, phone, id_number, address, birth_date — to any authenticated user, confirmed live with a throwaway pair of accounts. Both policies dropped; `getLeaderboard()` already read `leaderboard_profiles` (safe columns), `getMyTeamLeaderboard()` now reads the equivalent `team_profiles` view for the teammate case.
...
5. RESOLVED (`0015_privacy_hardening.sql`): the original bug here (`deleteUserAccount` filtering `health_scores` by `id` instead of `user_id`) was already fixed by the time of this audit, but the per-table delete loop it was part of had since broken a different way — `team_members`' direct DELETE grant for `authenticated` was revoked in `0013` (closing the hole in #2 above), so account deletion failed on the very first table and never even reached `health_scores`/`profiles`. Replaced the whole loop with `delete_own_account_data()`, a SECURITY DEFINER RPC that deletes only the `profiles` row; every user-owned table's `ON DELETE CASCADE` (verified against the live schema, not assumed) handles the rest regardless of the calling role's own per-table grants.
6. RESOLVED (`0002_user_handles.sql`, superseded by `0013_team_request_approval.sql`): team invites used `profiles.select().eq('email', …)`, which RLS makes return nothing unless the target opted into a public score — so inviting almost anyone failed. Replaced by a `profiles.handle` column (short random code, e.g. `swift-lotus-73`, server-assigned in `handle_new_user()`) plus SECURITY DEFINER RPCs — `find_profile_by_handle(text)` (preview: name + avatar only) and, as of 0013, `request_team_member_by_handle(text)` (resolves handle, files a *pending* request + notifies the target instead of linking accounts instantly — the old instant-add `add_team_member_by_handle` is dropped) and `respond_team_request(uuid, boolean)` (the target accepts/declines; only then are the mutual `team_members` rows created). All `EXECUTE`-granted to `authenticated` only. Frontend: `src/services/teamInvite.ts`, QR/manual UI in `src/components/team/TeamInvite.tsx` + `PendingRequests.tsx`, deep link `/?add=<handle>` handled in `App.tsx`.
