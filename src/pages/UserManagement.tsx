...
export default function UserManagement({ onNavigate }: { onNavigate?: (page: string) => void }) {
  ...
  return (
    <div className="user-management-page">
      ...
      {isOffline && (
        <div style={{ background: '#fff9c4', padding: '10px 16px', borderRadius: '8px', marginBottom: '16px', fontSize: '14px', color: '#856404', border: '1px solid #ffeeba', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span>⚠️ <b>Offline Mode:</b> Connection to backend failed. Showing mock data. Changes will be queued for sync.</span>
        </div>
      )}

      {isSupabaseConfigured() && !isOffline && (
        <div style={{ background: '#e8f5e9', padding: '8px 16px', borderRadius: '8px', marginBottom: '16px', fontSize: '12px', color: '#2e7d32', border: '1px solid #c8e6c9', display: 'flex', alignItems: 'center', gap: '8px' }}>
      ...
      {showForm && (
        <form className="user-form" onSubmit={handleSubmit}>
          <div className="form-header">
            ...
            <button type="button" className="icon-close" onClick={handleCancel}>✕</button>
          </div>
